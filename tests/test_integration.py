...
def test_hydrogel():
    ...
    assert int(df.loc[1, ['voronoi_area']].to_numpy()[3][0]) == int(918), df.loc[1, ['voronoi_area']].to_numpy()[3][0]  # 'tested value in hydrogel df incorrect'
    os.remove(output_video)
    os.remove(output_df)
    if os.path.exists(temp_dir):
        shutil.rmtree(temp_dir)
...
def test_bacteria():
    ...
    assert int(df.loc[1, ['box_width']].to_numpy()[0][0]) == int(
        4.0), df.loc[1, ['box_width']].to_numpy()[0][0]
    os.remove(output_video)
    os.remove(output_df)
    if os.path.exists(temp_dir):
        shutil.rmtree(temp_dir)
...
def test_discs():
    ...
    assert df.loc[1, ['x']].to_numpy()[0][0] == 1019.5, df.loc[1, ['x']].to_numpy()[
